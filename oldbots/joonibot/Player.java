	...
	public static void main(String[] args){
		...
		if (gc.planet() == Planet.Earth){
			...
			while (true){
				...
				// Remove empty but visible rally points, leaving the most recent one if there aren't any left
				...
				// Set rally points to enemy factories whenever possible
				...
				for(int i = 0; i < units.size(); i++){
					...
					if (u.unitType() == UnitType.Ranger && gc.round() > 100)
						bot.act2();
					else if (bot != null)
						bot.act();
				}
		...
		// Mars
			...
			while (true){
				...
				// Remove empty visible rally points, leaving the most recent one if there aren't any left
				for(int i = 0; i < rallyPoints.size(); i++){
					MapLocation rLoc = bc.bcMapLocationFromJson(rallyPoints.peek());
					if (gc.canSenseLocation(rLoc) && gc.senseNearbyUnitsByTeam(rLoc,0,enemyTeam).size() == 0)
						rallyPoints.poll();
